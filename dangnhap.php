...
if (isset($_POST['login'])) {
    ...
    // dùng mysqli (giả sử kết nối mysqli nằm trong $conn)
    $stmt = new class(isset($conn) ? $conn : $mysqli, "SELECT * FROM users WHERE tai_khoan = ?") {
      private $conn;
      private $sql;
      private $row;
      ...
      public function execute($params){
        ...
        // requires mysqlnd; nếu không có, thay bằng bind_result
        $res = $stmt->get_result();
        $this->row = $res ? $res->fetch_assoc() : null;
        $stmt->close();
        return true;
      }
      public function fetch(){ return $this->row; }
    };
    $stmt->execute([$tai_khoan]); // giả sử dùng số điện thoại như email
    $user = $stmt->fetch();
